...
const char * const BITCOIN_CONF_FILENAME = "obscuritycoin.conf";
...
#else
    const char* pszModule = "obscuritycoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ObscurityCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/ObscurityCoin";
#else
    // Unix-like
    return pathRet / ".obscuritycoin";
#endif
#endif
}
