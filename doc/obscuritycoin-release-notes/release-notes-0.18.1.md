ObscurityCoin Core version 0.18.1 is now available from:

  <https://download.obscuritycoin.org/obscuritycoin-0.18.1/>
...
`/Applications/ObscurityCoin-Qt` (on Mac) or `obscuritycoind`/`obscuritycoin-qt` (on
Linux).
...
ObscurityCoin Core is supported and extensively tested on operating systems
using the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not
recommended to use ObscurityCoin Core on unsupported systems.
...
10.10. Additionally, ObscurityCoin Core does not yet change appearance when
...
- The `rpcallowip` option can no longer be used to automatically listen
  ...
  [...] -p 127.0.0.1:14545:14545` (this is an extra `:14545` over the
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](mailto:contact@obscuritycoin.org) the ObscurityCoin Core
  project to let us know about their use-case, as this feature could be
  deprecated in the future).

Wallet changes
--------------
When creating a transaction with a fee above `-maxtxfee` (default 0.1 OBSC),
the RPC commands `walletcreatefundedpsbt` and  `fundrawtransaction` will now fail
instead of rounding down the fee. Beware that the `feeRate` argument is specified
in OBSC per kilobyte, not litoshi per byte.
...
- A new [document](/blob/master/doc/obscuritycoin-conf.md)
  about the `obscuritycoin.conf` file describes how to use it to configure
  ObscurityCoin Core.

- A new document introduces ObscurityCoin Core's BIP174 [Partially-Signed
  ObscurityCoin Transactions
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent ObscurityCoin-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  ObscurityCoin Core to libssl vulnerabilities in the past, builders who don't
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when ObscurityCoin Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of ObscurityCoin Core, subsequently importing
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart obscuritycoind with the `-deprecatedrpc=generate`
...
- A new `obscuritycoin-wallet` tool is now distributed alongside ObscurityCoin
...
This section describes planned changes to ObscurityCoin Core that may affect
other ObscurityCoin software and services.

- Since version 0.16.0, ObscurityCoin Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with ObscurityCoin Core 0.20 (expected about
  a year after 0.18), ObscurityCoin Core will default to native segwit
  ...
  to bech32 addresses, and if the ObscurityCoin Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in ObscurityCoin Core 0.20.
  ...
  set the `addresstype=bech32` configuration option in any ObscurityCoin Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, ObscurityCoin Core
...
- A sub-project of ObscurityCoin Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with ObscurityCoin Core.  See their [project
...
- This release changes the Random Number Generator (RNG) used from
  OpenSSL to ObscurityCoin Core's own implementation, although entropy
  gathered by ObscurityCoin Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves ObscurityCoin Core a
...
- On macOS, ObscurityCoin Core now opts out of application CPU throttling
...
- #14545 Let wallet `importmulti` RPC accept labels for standard scriptPubKeys (ryanofsky)
