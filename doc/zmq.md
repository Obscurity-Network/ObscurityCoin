...
The ObscurityCoin Core daemon can be configured to act as a trusted "border
router", implementing the obscuritycoin wire protocol and relay, making
...
The ZeroMQ feature in ObscurityCoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building obscuritycoind:
...
For instance:
    ...
    $ obscuritycoind -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/obscuritycoind.tx.raw \
...
These options can also be provided in obscuritycoin.conf.
...
From the perspective of obscuritycoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into obscuritycoind directly. Furthermore, no information is
...
using. ObscurityCoind appends an up-counting sequence number to each
